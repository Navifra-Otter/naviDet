...
def convert_to_coco_api(ds):
    ...
    for img_idx in range(len(ds)):
        ...
        if "keypoints" in targets:
            ...
            # Explicit flat shape so N=0 (no annotations on this image) works.
            n_obj = keypoints.shape[0]
            flat = int(keypoints.numel() // n_obj) if n_obj else 0
            keypoints = keypoints.reshape(n_obj, flat).tolist()
        num_objs = len(bboxes)
