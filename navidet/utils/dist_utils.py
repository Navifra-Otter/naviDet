...
# DataLoader imported lazily inside warp_loader to avoid a circular import
# (navidet.data depends on the registry, which imports BaseConfig, which uses dist_utils).
...
def warp_loader(loader, shuffle=False):
    from navidet.data import DataLoader  # local import: see top-of-file note
    if is_dist_available_and_initialized():
...
# Aliases — ecpose code calls the function `is_dist_avail_and_initialized`
# while ecdetseg uses the longer name. Expose both.
is_dist_avail_and_initialized = is_dist_available_and_initialized
