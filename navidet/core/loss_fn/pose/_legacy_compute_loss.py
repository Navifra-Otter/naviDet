...
from navidet.utils.bbox import point2box_xywh, bbox_iou
...
class ComputeLoss:
    def __init__(self, model, use_focal=True, use_vari_focal=True, kpt_loss_type='hybrid'):
        ...
        if use_focal:
            if use_vari_focal:
                from navidet.core.loss_fn.classify.pose import VarifocalLoss
        ...
        if kpt_loss_type == 'oks':
            from navidet.core.loss_fn.pose.pose import KeypointLoss
            self.kpt_loss_fn = KeypointLoss(kpt_sigmas)
        elif kpt_loss_type == 'improved':
            from navidet.core.loss_fn.pose.pose import ImprovedKeypointLoss
            self.kpt_loss_fn = ImprovedKeypointLoss(kpt_sigmas)
        elif kpt_loss_type == 'robust':
            from navidet.core.loss_fn.pose.pose import RobustKeypointLoss
            self.kpt_loss_fn = RobustKeypointLoss(kpt_sigmas)
        else:  # 'hybrid'
            from navidet.core.loss_fn.pose.pose import HybridKeypointLoss   
