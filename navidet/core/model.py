...
모델 정의 — 한 백본/넥을 공유하는 두 계열 모델을 함께 제공한다.
  ...
  · Det6DoF : 백본 + PAN 넥 + (Pose6DoFHead + DepthHead)  — 1-stage 6DoF Pose
  · MultiTaskDet : 백본 + PAN 넥 + MultiTaskHead               — 2D Detect/Segment/Pose

`build_model(task=...)`로 둘 중 하나를 빌드한다 (task ∈ {6dof, detect, segment, pose}).

Det6DoF (Direction B) 구조:
...
class MultiTaskDet(nn.Module):
    """
    2D 멀티태스크 모델 (Detection + Segmentation + Pose).
...
#   seg/pose 모델은 박스가 필요하므로 detection을 기본 포함한다(표준 관례).
TASK_PRESET = {
    "detect": ("detect",),
...
def build_model(task: str = "6dof", nc: int = 13, scale: str = "n",
    ...
    통합 빌더. task로 모델 계열을 선택한다.
      ...
      task == "6dof"             → Det6DoF (Pose6DoFHead + DepthHead)
      task in {detect,segment,pose} → MultiTaskDet (MultiTaskHead, 해당 task 프리셋)

    예) 6DoF LINEMOD 13객체 → build_model("6dof", nc=13)
        키포인트 pose          → build_model("pose", nc=3, kpt_shape=(4,3))
    """
    if task == "6dof":
        return Det6DoF(nc=nc, scale=scale, rot_repr=rot_repr, light_head=light_head)
    assert task in TASK_PRESET, f"task must be '6dof' or one of {list(TASK_PRESET)}"
    return MultiTaskDet(nc=nc, scale=scale, tasks=TASK_PRESET[task], nm=nm,
                    kpt_shape=kpt_shape)
