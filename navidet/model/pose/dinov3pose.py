...
from navidet.core.nn.modules.block import FeatureAdaptor, SPPF, PAN
from navidet.core.head.pose import PoseHead
...
class DINOv3Pose(nn.Module):
    def __init__(self, backbone='dinov3_convnext_base', 
        ...
        if 'convnext' in model_name:
            ...
            from navidet.model.utils import convnext_sizes, convnext_ckps, DINO_CONVNEXT_MODELS
            self.tc = convnext_sizes[model_size]["dims"][:-1]
            
            self.backbone = Dinov3ConvNext(
                backbone=DINO_CONVNEXT_MODELS[model_size],
                source='local',
    ...
    def forward_features(self, x):
        feature_list = self.backbone.get_feature_spaces(x)
